...
if __name__ == '__main__':
    ...
    try:
        ...
        port = int(os.environ.get("PORT", 7860))  # Spaces expects port 7860
        print(f"🚀 Serving on http://0.0.0.0:{port}")
        serve(app, host="0.0.0.0", port=port)
    except Exception as e:
        print(f"❌ Failed to start Waitress server: {e}")
        print("Make sure Waitress is installed: pip install waitress")
        print("You can also try running on a different port by changing the PORT environment variable.")